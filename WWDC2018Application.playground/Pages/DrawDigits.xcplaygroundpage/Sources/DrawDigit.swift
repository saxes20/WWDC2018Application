...
class Canvas: UIImageView {
    ...
    func predictLetter() -> Array<Any>.Index {
    ...
    func predictImage (img: UIImage) -> Array<Any>.Index? {
        ...
        guard let throw_output = try? MLMultiArray(shape:[10], dataType:.double) else {
            fatalError("Unexpected runtime error. MLMultiArray")
        }
        
        guard let prediction = try? model.prediction(input1: input_data) else { return 0 }
...
public class DrawView: UIView {
    ...
    @objc func accept (_ sender: UIButton) {
        print("Accepted")
        digits += predictionLabel.text!
        digits += " "
        PlaygroundPage.current.keyValueStore["digits"] = .string(digits)
        reset()
    }
